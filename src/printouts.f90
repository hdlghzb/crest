...
subroutine confscript_morehelp(flag)
  ...
  case ('qcg')
    ...
    call help_opt('--qcg-final-method <method>',fw,'QCG grow-final method: inherit, --gfn1, --gfn2, or --gfnff')
    call help_opt('--qcg-final-opt-level <level>',fw,'Optimization level for QCG grow-final only')
    call help_opt('--qcg-final-cinp <file>',fw,'Final-only numeric QCG structural restraints')
    write (stdout,*)
